...
def mod_unet(config):
    ...
    c7 = Conv2D(512, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(p5)
    c7 = Dropout(0.3)(c7)
    c7 = Conv2D(512, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(c7)
    
    u9 = Conv2DTranspose(256, (2, 2), strides=(2, 2), padding='same')(c7)
...
def kuc_vnet(config):
    ...
    output = Conv2D(config['num_classes'], kernel_size = (1,1), name="out", activation = 'softmax',dtype="float32")(x)
...
def get_model_transfer_lr(model, num_classes):
    ...
    output = Conv2D(num_classes, kernel_size = (1,1), name="out", activation = 'softmax')(x) 
    model = Model(inputs = model.input, outputs=output) 
